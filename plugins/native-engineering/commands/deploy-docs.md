...
```bash
# Count components
echo "Agents: $(ls plugins/native-engineering/agents/*.md | wc -l)"
echo "Commands: $(ls plugins/native-engineering/commands/*.md | wc -l)"
echo "Skills: $(ls -d plugins/native-engineering/skills/*/ 2>/dev/null | wc -l)"

# Validate JSON
cat .claude-plugin/marketplace.json | jq . > /dev/null && echo "✓ marketplace.json valid"
cat plugins/native-engineering/.claude-plugin/plugin.json | jq . > /dev/null && echo "✓ plugin.json valid"

# Check all HTML files exist
for page in index agents commands skills mcp-servers changelog getting-started; do
  if [ -f "plugins/native-engineering/docs/pages/${page}.html" ] || [ -f "plugins/native-engineering/docs/${page}.html" ]; then
...
```bash
git status --porcelain plugins/native-engineering/docs/
```
...
on:
  push:
    branches: [main]
    paths:
      - 'plugins/native-engineering/docs/**'
  workflow_dispatch:
  ...
  deploy:
    ...
    steps:
      ...
      - uses: actions/upload-pages-artifact@v3
        with:
          path: 'plugins/native-engineering/docs'
