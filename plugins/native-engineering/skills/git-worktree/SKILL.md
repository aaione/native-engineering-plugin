...
1. **Code Review (`/workflow:review`)**: If NOT already on the PR branch, offer worktree for isolated review
2. **Feature Work (`/workflow:work`)**: Always ask if user wants parallel worktree or live branch work
...
The skill is automatically called from `/workflow:review` and `/workflow:work` commands:
