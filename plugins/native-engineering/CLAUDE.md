# Native Compounding Engineering Plugin Development
...
commands/
├── workflows/  # Core workflow commands (workflow:plan, workflow:review, etc.)
└── *.md        # Utility commands
...
**Workflow commands** use `workflow:` prefix to avoid collisions with built-in commands:
...
- `/workflow:compound` - Document solved problems

**Why `workflow:`?** Claude Code has built-in `/plan` and `/review` commands. Using `name: workflow:plan` in frontmatter creates a unique `/workflow:plan` command with no collision.
