...
This repository is a Claude Code plugin marketplace that distributes the `native-engineering` plugin to developers building with AI-powered tools.
...
└── plugins/
    └── native-engineering/   # The actual plugin
        ├── .claude-plugin/
...
## Philosophy: Native Compounding Engineering

**Each unit of engineering work should make subsequent units of work easier—not harder.**

When working on this repository, follow the native compounding engineering process:
...
### Updating the Native Compounding Engineering Plugin
...
```bash
# Count agents
ls plugins/native-engineering/agents/*.md | wc -l

# Count commands
ls plugins/native-engineering/commands/*.md | wc -l

# Count skills
ls -d plugins/native-engineering/skills/*/ 2>/dev/null | wc -l
```
...
- [ ] `plugins/native-engineering/.claude-plugin/plugin.json` → `description` field
- [ ] `.claude-plugin/marketplace.json` → plugin `description` field
- [ ] `plugins/native-engineering/README.md` → intro paragraph
...
- [ ] `plugins/native-engineering/.claude-plugin/plugin.json` → `version`
- [ ] `.claude-plugin/marketplace.json` → plugin `version`

#### 4. Update documentation

- [ ] `plugins/native-engineering/README.md` → list all components
- [ ] `plugins/native-engineering/CHANGELOG.md` → document changes
...
cat plugins/native-engineering/.claude-plugin/plugin.json | jq .
```
...
grep -o "Includes [0-9]* specialized agents" plugins/native-engineering/.claude-plugin/plugin.json
ls plugins/native-engineering/agents/*.md | wc -l
```
...
2. Install the plugin:
   ...
   ```bash
   claude /plugin install native-engineering
   ```
...
1. Create `plugins/native-engineering/agents/new-agent.md`
...
1. Create `plugins/native-engineering/commands/new-command.md`
...
1. Create skill directory: `plugins/native-engineering/skills/skill-name/`
...
- Use: `ai-powered`, `native-engineering`, `workflow-automation`, `knowledge-management`
